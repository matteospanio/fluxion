...
- **Progress (2026-07-01) — differentiable-training thread closed to the public API.**
  ...
  - **G8** (node) DONE: `RtGraph::Fir` — alloc-free direct-form FIR realtime node (`y[n]=Σ taps[k]·x[n-k]`), streams == offline `fir_filter`. (The `op_rt`/graph-op wiring is deferred with D13 — FIR isn't an OpKind yet because variable-length taps don't fit the scalar param model.)
