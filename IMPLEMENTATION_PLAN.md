...
- **Progress (2026-07-01) — differentiable-training thread closed to the public API.**
  ...
  - **C3 + C6** DONE (paper prerequisite P-1): the SIMD interleaved kernel is now the **product** batch path, with runtime ISA dispatch. `sos_filter` fused in place (one buffer, no per-section allocation); `sos_filter_interleaved` gained a runtime-AVX2 monomorphization (`is_x86_feature_detected!`, AVX2-only — no FMA contraction, results bit-identical; portable baseline build) + a stateful `_chunk` variant carrying DF2T state across time blocks. `fluxion_backend::sos_filter_batch` = 16-row groups × 4096-frame time blocks (gather → SIMD cascade with carried state → scatter; the hot buffer confines the power-of-two-stride transposes) parallelized over groups with rayon; `Cpu::filter` goes channel-parallel above a work cutoff; `process_batch`'s fallback is `par_iter`. Measured on an i5-1235U laptop: batch 110 → **743 Msamples/s** (6.8×, bit-identical; single-thread 327; pure kernel 1005+); 8-ch × 60 s `process` 179 → **62 ms**. Frame-outer transpose ordering measured ~35% slower (2 MB power-of-two row stride aliases one cache set) — documented in-code.
...
- **Progress (2026-07-03) — SoX-substitute CLI + distribution readiness (multi-agent wave).**
  ...
  - **J13 full slice (2026-07-05):** Rust converter `fluxion-io::checkpoint` (feature
    `checkpoint`) replays FLAMO `SOSFilter`/`SVF` (every `filter_type`, incl. the upstream
    shelving dead-code quirk)/`Biquad` and torchfx.ddsp learnable filters; CLI verb
    `fluxion import` (certify gate + `--project-stable` Jury clamp) writes a raw-`biquad` `.fxg`;
    Python `fluxion.interop.import_checkpoint` parses `.safetensors`/`.pt`/`.onnx`/torchfx
    compiled artifacts and calls the same Rust converter (`_fluxion.import_state_dict` /
    `save_biquad_fxg`). Golden-tested against 15 real FLAMO + torchfx checkpoints
    (`crates/fluxion-io/tests/fixtures/checkpoints/`, generated by the libraries themselves).
    Still out: MIMO banks, FIR taps, FLAMO GEQ/PEQ/AccurateGEQ (clear errors point at the
    supported paths).
