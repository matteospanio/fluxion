...
def test_eager_chain_shape_and_dtype():
    ...
    chain = fluxion.filter.Lowpass(1000.0, 4) | fluxion.effect.Gain(0.5)
...
def test_lowpass_attenuates_high_frequency():
    ...
    y = fluxion.filter.Lowpass(1000.0, 6).process(hi, FS)
...
def test_output_is_array_api_compliant():
    ...
    y = fluxion.effect.Gain(2.0).process(np.arange(8, dtype=np.float32), FS)
    xp = compat.array_namespace(y)
    assert xp is not None
    # round-trip a fluxion output back into a chain via the Array-API namespace.
    np.testing.assert_allclose(np.asarray(fluxion.effect.Gain(0.5).process(xp.asarray(y), FS)), y * 0.5, atol=1e-6)
...
def test_dlpack_torch_roundtrip():
    ...
    y = fluxion.filter.Lowpass(1000.0, 4).process(x, FS)  # torch tensor in, no .numpy()
    yt = torch.from_dlpack(y)  # numpy out → torch, zero-copy
    ref = fluxion.filter.Lowpass(1000.0, 4).process(x.numpy(), FS)
...
def test_lowpass_parity_with_scipy():
    ...
    y = fluxion.filter.Lowpass(fc, order).process(x, fs)
...
def test_cheby2_parity_with_scipy():
    ...
    y = fluxion.filter.Cheby2Lowpass(fc, order, rs).process(x, fs)
...
def test_process_2d_multichannel():
    ...
    chain = fluxion.filter.Lowpass(2000.0, 4)
...
def test_torch_sos_module_is_trainable():
    ...
    m = fxt.SosModule.from_chain(fluxion.filter.Lowpass(2000.0, 4), FS)
...
def test_process_batch_matches_per_row():
    ...
    chain = fluxion.filter.Lowpass(1500.0, 6) | fluxion.filter.Highpass(200.0, 2)  # pure filter → SIMD batch path
...
def test_process_batch_fallback_nonfilter():
    ...
    y = fluxion.effect.Gain(2.0).process_batch(x, FS)
    np.testing.assert_allclose(y, 2.0 * x, atol=1e-6)


def test_process_batch_rejects_1d():
    with pytest.raises(ValueError):
        fluxion.effect.Gain(1.0).process_batch(np.zeros(10, dtype=np.float32), FS)
...
def test_augment_determinism_and_range():
    ...
    make = lambda: Compose(
        [RandomChain(fluxion.filter.Lowpass, cutoff=(200.0, 8000.0), order=4, p=1.0, rng=42)]
    )
    np.testing.assert_array_equal(make()(x, FS), make()(x, FS))

    # Sampled params land in the requested range; the fixed param stays fixed.
    rc = RandomChain(fluxion.filter.Lowpass, cutoff=(200.0, 8000.0), order=4, rng=7)
    for _ in range(200):
        p = rc.sample()
...
def test_augment_p_zero_is_identity():
    ...
    rc = RandomChain(fluxion.effect.Gain, gain=(0.1, 0.9), p=0.0, rng=1)
...
def test_augment_integer_range_samples_ints():
    ...
    rc = RandomChain(fluxion.filter.Lowpass, cutoff=1000.0, order=(2, 8), rng=3)
    for _ in range(50):
        o = rc.sample()["order"]
