...
class Chain:
    ...
    def render_region(
        self, x: F32, fs: int, start: int, end: int, automation: Automation | None = ...
    ) -> F32:
        """Render frames ``[start, end)`` — bit-identical to that window of a whole render.

        Costs ``end`` frames of work, not ``end - start``: every op in front of the window
        carries state, so it has to be computed. Raises for a chain containing an op that needs
        the whole signal (``normalize``, ``loudnorm``, ``reverse``, ``limiter``).
        """
        ...

    def automate(self, x: F32, fs: int, automation: Automation) -> F32:
        """Render with automation driving the chain's parameters."""
        ...
...
class Automation:
    """Curves driving named nodes of a chain. Chainable; each method returns a new instance."""

    def __new__(cls) -> Automation: ...
    def ramp(
        self, node: str, param: str, start: float, end: float, seconds: float
    ) -> Automation: ...
    def db_ramp(
        self, node: str, param: str, start: float, end: float, seconds: float
    ) -> Automation:
        """A fade at a constant rate in decibels — half of a 60 dB fade is -30 dB, not -6."""
        ...

    def lfo(
        self, node: str, param: str, rate_hz: float, low: float, high: float, phase: float
    ) -> Automation: ...
    def points(self, node: str, param: str, points: list[tuple[float, float]]) -> Automation: ...
