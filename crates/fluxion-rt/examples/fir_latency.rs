...
//! same callback-body timing, but paced (one block per buffer period, as in
//! `soundlamp_demo`) and pure FIR (no SetCoeffs automation — a FIR node has
//! no live coefficient swap). Emits one JSON line per buffer size.
//!
//! Usage: fir_latency [seconds-per-config] [taps.f32]
//!   taps.f32 = headerless little-endian f32 taps, natural order h[0..N-1];
//!   omitted -> synthetic 1024 taps.
...
fn main() {
    ...
    let taps: Vec<f32> = match args.get(2) {
        Some(path) => std::fs::read(path)
            .expect("read taps file")
            .chunks_exact(4)
            .map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]]))
            .collect(),
        None => (0..TAPS).map(|k| ((k as f32) * 0.013).sin() / 64.0).collect(),
    };
    ...
    for buffer in [128usize, 256, 512] {
        ...
        println!(
            "{{\"case\":\"fir_latency\",\"taps\":{TAPS},\"buffer\":{buffer},\"fs\":{FS},\
